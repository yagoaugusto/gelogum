...
try {
    ...
    if ($status === 'separated') {
        gelo_flash_set('success', 'Pedido já estava como separado.');
        gelo_redirect(GELO_BASE_URL . '/withdrawal.php?id=' . $id);
    }
    if ($status !== 'requested') {
        gelo_flash_set('error', 'Status inválido para separação.');
        gelo_redirect(GELO_BASE_URL . '/withdrawal.php?id=' . $id);
    }

    $pdo->prepare("UPDATE withdrawal_orders SET status = 'separated', separated_at = NOW(), separated_by_user_id = :actor WHERE id = :id")
        ->execute(['actor' => $actorId, 'id' => $id]);

    // Disparo WPP (best-effort)
    try {
        gelo_whatsapp_notify_order($id, $status !== '' ? $status : null, 'separated');
    } catch (Throwable $ignored) {
        // ignore
    }

    gelo_flash_set('success', 'Pedido marcado como separado.');
    gelo_redirect(GELO_BASE_URL . '/withdrawal.php?id=' . $id);
} catch (Throwable $e) {
